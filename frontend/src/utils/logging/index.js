...
class LoggingService {
  constructor() {
    // 개발 환경에서는 INFO 레벨부터, 프로덕션 환경에서는 ERROR 레벨부터 로깅
  ...
  _log(level, module, message, data) {
    ...
    // 콘솔 출력
    const prefix = `[${timestamp.split('T')[1].substring(0, 8)}] [${this._getLogLevelName(level)}] [${module || 'App'}]`;
  ...
  clearLogs() {
    this.recentLogs = [];
    this.debug('LoggingService', '로그 초기화됨');
  }
}
