...
class SocketIOManager:
    ...
    def __init__(self, user_service: Optional[UserService] = None):
        ...
        # 내부 이벤트 버스 핸들러 등록
        self.event_bus.register('session_cleanup', self.handle_session_cleanup_event)
    ...
    async def _handle_connect(self, sid, environ, auth):
        ...
        try:
            ...
            if not success:
                ...
                await self.emit_message(WSMessageType.ERROR,
                                        {"message": f"유효하지 않은 인증 토큰입니다: {user_or_error}", "code": "invalid_auth_token"},
                                        target_sid=sid)
                return
            ...
            # 내부 이벤트 발행
            await self.event_bus.emit('user_connected', {
    ...
    async def _handle_disconnect(self, sid: str) -> None:
        ...
        if user_id:
            ...
            # 내부 이벤트 발행
            await self.event_bus.emit('user_disconnected', {'user_id': user_id, 'sid': sid, 'session_id': session_id})
        else:
    ...
    async def _handle_cve_unsubscribe(self, sid: str, data: Dict[str, Any]) -> None:
        ...
        # 결과 처리 및 응답/이벤트 발생
        if removed_from_user or removed_from_session:
             ...
             if removed_from_cve_list: await self.broadcast_subscribers_updated(cve_id)
        else:
            logger.warning(f"CVE 구독 해제 요청 처리됨 (변경 없음) - 사용자: {user_id}, CVE: {cve_id}, 세션: {session_id}, SID: {sid}")
            await self.emit_message(WSMessageType.SUBSCRIPTION_STATUS, {
                 "status": "not_subscribed", "cve_id": cve_id, "session_id": session_id,
                 "success": True, "message": f"CVE {cve_id}는 이미 구독 중이 아님"
             }, target_sid=sid)
    ...
    async def broadcast_subscribers_updated(self, cve_id: str):
        ...
        count = len(self.cve_subscribers.get(str(cve_id), set()))
        logger.info(f"CVE 구독자 수 변경 알림 - CVE: {cve_id}, 구독자 수: {count}")
        await self.emit_message(WSMessageType.SUBSCRIBERS_UPDATED,
                                {'cve_id': cve_id, 'subscribers_count': count},
                                target_cve=cve_id)
    ...
    async def cleanup_stale_connections_and_subscriptions(self):
        ...
        for session_id in list(inactive_sessions):
             ...
             if session_id in self.session_cve_subscriptions:
                  cves_in_session = list(self.session_cve_subscriptions[session_id])
                  del self.session_cve_subscriptions[session_id]
                  cleaned_sessions += 1
                  logger.debug(f"세션 {session_id}의 CVE 구독 {len(cves_in_session)}개 정리됨")
    ...
    async def handle_session_cleanup_event(self, data: Dict[str, str]):
        """
        EventBus를 통해 전달된 'session_cleanup' 이벤트를 처리합니다.
        주어진 세션 ID에 해당하는 세션 구독 정보를 정리합니다.
        """
        session_id = data.get('session_id')
        user_id = data.get('user_id') # 요청자 정보 (로깅용)

        if not session_id:
            logger.warning("EventBus: session_cleanup 이벤트에 session_id 없음")
            return

        logger.info(f"EventBus: session_cleanup 이벤트 수신 - 세션: {session_id}, 요청 사용자: {user_id}")

        async with self.cleanup_lock:
            if session_id in self.session_cve_subscriptions:
                 subscribed_cves = list(self.session_cve_subscriptions[session_id])
                 logger.info(f"세션 구독 정리 시작 - 세션: {session_id}, 정리할 CVE 수: {len(subscribed_cves)}")

                 # 세션 구독 목록에서 해당 세션 ID 항목 제거
                 del self.session_cve_subscriptions[session_id]

                 # 참고: 이 세션 정리로 인해 특정 CVE의 구독자가 0명이 될 수 있으나,
                 # 해당 CVE 구독자 목록 업데이트는 주기적 정리 작업(cleanup_stale...)에서 처리됩니다.
                 # 여기서 즉시 처리하려면 추가 로직 필요.

                 logger.info(f"세션 구독 정보 제거 완료 - 세션: {session_id}")
            else:
                 logger.info(f"정리할 세션 구독 정보 없음 - 세션: {session_id}")
